...
parser = argparse.ArgumentParser(description='3d cube in 3-layer fc net')
...
parser.add_argument('--dataset', default='3dcube', type=str,
...
def generate_data(num_sample, digit_list):

    train_data, train_label = [], []

    for digit in digit_list:
        train_data.append(torch.Tensor(num_sample, 3).uniform_(-1, 1) + MEAN[digit])
        train_label.append(torch.ones(num_sample) * digit)
    train_data = torch.cat(train_data)
    train_label = torch.cat(train_label).long()
...
def get_data(dataset, digit_list, train=True):
    ...
    train_data, train_label = generate_data(num_sample, digit_list)
        ...
        for idx, digit in enumerate(digit_list):
            ...
            if mean_stored[idx].size(0) == 0:
                ...
                if feature_selected.size(0) == 0:
                    ...
                    mean = MEAN[digit].view(1, 3).to(device)
...
def train(train_loader, val_loader, train_epoch, mode, lr, dataset, digit_list):
    ...
    torch.save(net.state_dict(), '3dcube_results/model/' +
...
if __name__ == '__main__':
    ...
    digit_list = [int(i) for i in range(8)]
    ...
    best_net.load_state_dict(torch.load(
        '3dcube_results/model/' + mode + '_{}_{}_{}_best_{}.pkl'.format(args.network, args.dir_num, args.trial, args.validate)))
    best_net.eval()
    best_net.to(device)
